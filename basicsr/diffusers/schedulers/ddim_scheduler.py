...
from basicsr.diffusers.utils.sugar import betas_for_alpha_bar
...
class EditDDIMScheduler:
    ...
    def step(
        ...
        if isinstance(model_output, dict):
            pred = model_output['pred'] # type: ignore
            model_output = model_output['eps']  # type: ignore
        elif model_output.shape[1] == sample.shape[
                1] * 2 and self.variance_type in ['learned', 'learned_range']:
            model_output, _ = torch.split(model_output, sample.shape[1], dim=1)  # type: ignore
        else:
        ...
        if self.clip_sample:
            pred_original_sample = torch.clamp(pred_original_sample, -1, 1)  # type: ignore
        ...
        if eta > 0:
            device = model_output.device if torch.is_tensor( # type: ignore
            ...
            if not torch.is_tensor(model_output):
                variance = variance.numpy()  # type: ignore
